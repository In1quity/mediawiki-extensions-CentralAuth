...
abstract class CentralAuthTokenSessionProviderTest extends MediaWikiIntegrationTestCase {
	...
	/**
	 * Patch our modified session store into CentralAuthServices
	 */
	private function patchSessionStore() {
		$this->services = TestingAccessWrapper::newFromObject( CentralAuthServices::getInstance() );
		$this->services->sessionStore = $this->sessionStore;
	}
	...
	protected function makeUser( $id, $name ) {
		...
		if ( $this->userObjects === null ) {
			...
			$this->services->userFactory = $userFactory;
