...
class SpecialCentralAutoLogin extends UnlistedSpecialPage {
	...
	public function execute( $par ) {
		...
		case 'toolslist':
			...
			if ( !$user->isAnon() ) {
				$skin = $this->getSkin();
				if (
					!$this->centralAuthServices->isUIReloadRecommended( $user ) &&
		...
		case 'refreshCookies': // Refresh central cookies (e.g. in case 'remember me' was set)
			...
			$this->centralAuthServices->setP3P();
		...
		case 'checkLoggedIn': // Check if we're logged in centrally
			...
			$key = $this->centralAuthServices->memcKey( 'centralautologin-token', $token );
		...
		case 'createSession': // Create the local session and shared memcache token
			...
			if ( $token !== '' ) {
				// Load memc data
				$key = $this->centralAuthServices->memcKey( 'centralautologin-token', $token );
				$memcData = $this->centralAuthServices->getKeyValueUponExistence( $sessionStore, $key );
			...
			$token = MWCryptRand::generateHex( 32 );
			$key = $this->centralAuthServices->memcKey( 'centralautologin-token', $token, $wikiid );
			$sessionStore->set( $key, $memcData, $sessionStore::TTL_MINUTE );
		...
		case 'setCookies': // Check that memcached is validated, and set cookies
			...
			$this->centralAuthServices->setCentralSession( [
			...
			if ( !User::idFromName( $centralUser->getName() ) ) {
				...
				if ( $this->centralAuthServices->autoCreateUser( $user )->isGood() ) {
	...
	private function getCentralSession( $centralUser, $user ) {
		...
		$centralSession = $this->centralAuthServices->getCentralSession( $this->session );
		$request = $this->getRequest();
		...
		if ( !isset( $centralSession['sessionId'] ) ) {
			$centralSession['sessionId'] = $this->centralAuthServices->setCentralSession(
