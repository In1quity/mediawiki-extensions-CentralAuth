...
/**
 * Services that are useful in many parts of CentralAuth.
 *
 * @since 1.36
 */
class CentralAuthServices {
	/**
	 * Returns a CentralAuthServices object.
	 *
	 ...
	 * @return CentralAuthServices
	 */
	public static function getInstance( ContainerInterface $services = null ) : CentralAuthServices {
		return ( $services ?: MediaWikiServices::getInstance() )
			->getService( 'CentralAuth.CentralAuthServices' );
	}
	...
	public function scheduleCreationJobs( CentralAuthUser $centralUser ) {
		...
		foreach ( $this->config->get( 'CentralAuthAutoCreateWikis' ) as $wiki ) {
			...
			$job = Job::factory(
				...
				$this->titleFactory->makeTitleSafe( NS_USER, $name ),
